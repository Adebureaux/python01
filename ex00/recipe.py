...
class Recipe:
	...
	def __init__(self, name, cooking_lvl, cooking_time, ingredients, recipe_type, description=""):
		...
		if not isinstance(cooking_lvl, int) or cooking_lvl < 1 or cooking_lvl > 5:
			raise ValueError('cooking_lvl must be an integer that is between 1 and 5')
		self.cooking_lvl = cooking_lvl

		if not isinstance(cooking_time, int) or cooking_time < 0:
			raise ValueError('cooking_time must be a positive integer')
		self.cooking_time = cooking_time

		if not isinstance(ingredients, list) or not all(isinstance(item, str) for item in ingredients):
			raise TypeError('ingredients must be a list that contains only strings')
		self.ingredients = ingredients

		if recipe_type not in ['starter', 'lunch', 'dessert']:
			raise ValueError("recipe_type must be a string which can be 'starter', 'lunch' or 'dessert'")
