...
class Book:
	...
	def __init__(self, name, last_update, creation_date, recipes_list):
		...
		if not isinstance(last_update, datetime):
			raise TypeError('Book __init__ Book last_update must be a datetime object.')
		self.last_update = last_update

		if not isinstance(creation_date, datetime):
			raise TypeError('Book __init__ Book creation_date must be a datetime object.')
		self.creation_date = creation_date

		if not isinstance(recipes_list, dict) or not all(key in ['starter', 'lunch', 'dessert'] for key in recipes_list.keys()):
			raise ValueError("Book __init__ recipes_list must be a dictionary with keys that are either 'starter', 'lunch', or 'dessert'")
